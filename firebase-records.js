...
async function submitBehaviorRecord(payload) {
  ...
  const ref = await addDoc(collection(db, "behaviorRecords"), record);
  return { id: ref.id, uid: user.uid };
}

window.behaviorFirebase = {
  projectId: firebaseConfig.projectId,
  collectionName: "behaviorRecords",
  submitBehaviorRecord,
};

window.dispatchEvent(new CustomEvent("behavior-firebase-ready"));
