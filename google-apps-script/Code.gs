...
function doGet() {
  return json_({ ok: true, service: "pet-behavior-export", time: new Date().toISOString() });
}
...
function getConfig_(requestConfig) {
  ...
  const config = {
    ...
    folders: {
      "c-barq": requestConfig.folders?.["c-barq"] || props.getProperty("DRIVE_FOLDER_C_BARQ"),
      "short-cbarq":
        requestConfig.folders?.["short-cbarq"] || props.getProperty("DRIVE_FOLDER_SHORT_CBARQ"),
      "fe-barq": requestConfig.folders?.["fe-barq"] || props.getProperty("DRIVE_FOLDER_FE_BARQ"),
    },
  };
